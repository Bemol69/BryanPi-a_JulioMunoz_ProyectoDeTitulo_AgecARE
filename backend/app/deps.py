...
async def require_internal_key(request: Request) -> None:
    """Protege endpoints llamados servidor-a-servidor desde backend-general
    (no hay staff logueado en ese lado, así que no aplica un JWT de admin)."""
    key = request.headers.get("X-Internal-Key", "")
    if key != get_settings().internal_sync_key:
        raise unauthorized("Clave interna inválida.", "INVALID_INTERNAL_KEY")
