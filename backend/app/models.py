...
class CaregiverReview(Base):
    ...
    que alimenta el ranking del marketplace."""
    __tablename__ = "marketplace_caregiver_reviews"
    id: Mapped[uuid.UUID] = pk()
    ...
    job_reference: Mapped[str | None] = mapped_column(String(120), nullable=True)
    created_at: Mapped[datetime] = ts_now()


class CaregiverPointsLog(Base):
    """Historial de puntos de fidelización otorgados a una cuidadora.

    El total de puntos de una cuidadora es la suma de este historial (no se
    guarda como columna aparte para evitar que quede desincronizado).
    """
    __tablename__ = "marketplace_caregiver_points_log"
    id: Mapped[uuid.UUID] = pk()
    caregiver_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("marketplace_caregivers.id"), index=True)
    points: Mapped[int] = mapped_column(Integer)
    reason: Mapped[str] = mapped_column(String(20))  # job_completed | good_review | manual
    note: Mapped[str | None] = mapped_column(String(300), nullable=True)
    review_id: Mapped[uuid.UUID | None] = mapped_column(Uuid, nullable=True)
    awarded_by: Mapped[uuid.UUID | None] = mapped_column(Uuid, nullable=True)
    awarded_by_name: Mapped[str | None] = mapped_column(String(120), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), index=True)
